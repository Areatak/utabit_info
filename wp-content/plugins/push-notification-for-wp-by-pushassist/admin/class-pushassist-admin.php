...
END_SCRIPT;
    ...
    public static function pushassist_publish_new_post()
    {
		...
		$psaEditPostPush = $pushassist_settings['psaEditPostPush'];
		...
		if ('publish' === $post->post_status) {	//while updating post
			...
			if($psaEditPostPush === false){
				...
				printf('<label><input type="checkbox" value="1" id="pushassist-forced-checkbox" name="pushassist-donot-send-checkbox" style="margin: -3px 9px 0 1px;" />');
				_e('Don\'t Send Push Notification', 'push-notification-for-wp-by-pushassist');
				echo ' </label>';
			}

		} else if ('auto-draft' === $post->post_status) {	//while creating new post
	
			if($psa_auto_push === false){
        ...
        if (($psa_auto_push === false && 'publish' !== $post->post_status) || ('publish' === $post->post_status && $psaEditPostPush === false) || ('draft' === $post->post_status && $psa_auto_push === false)) {
    ...
    {		
        ...
        if ((!isset($_POST['pushassist-checkbox']) || !current_user_can('edit_posts') || isset($_POST['pushassist-donot-send-checkbox'])) && false === $psa_auto_push) {
    ...
    public static function send_pushassist_post_notification($new_status, $old_status, $post)
    {
        ...
        if (empty($post) || (isset($_POST['pushassist-donot-send-checkbox']))) {
